---
title: "2025年3月13日刷题记录"
published: 2025-03-13
description: "2025年3月13日刷题记录\r 问题1: 最大子数组之和  \r 给你一个整数数组 nums ，请你找出一个具有最大和的连续子数组（子数组最少包含一个元素），返回其最大和。  \r 子数组是数组中的一个连续部分。  \r \r 示例 1：  \r 输入：nums = [-2,1,-3,4,-1,2,1,-5,..."
tags: ["笔记", "数据结构", "算法", "刷题"]
category: "刷题日记"
draft: false
---
...
输出：3  
思路1:  哈希表思想
1. 先把所有 ≤0 的数，全部改成 n+1，排除干扰。  
2. 再遍历每个数，取绝对值 num，如果 num ≤n，就把下标 num-1 标为负数，表示这个数已出现。  
3. 最后遍历数组，第一个正数的下标 +1 就是答案；全负则返回 n+1。  
代码:  
```java
class Solution {
