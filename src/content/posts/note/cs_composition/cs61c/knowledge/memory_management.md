...
description: "内存管理\r 内存管理区域\r 栈区: 存放函数的参数值、局部变量等,栈帧（Stack Frame）是程序运行时在栈（Stack）上为每个活动的函数调用分配的一块内存区域。栈帧用于存储函数调用期间所需的各种信息，包括局部变量、函数参数、返回地址等。\r 堆区: 动态内存分配,是在程序运行时动态分配的内存区..."
tags: ["笔记", "计组", "CS"]
category: "计算机组成原理"
draft: false
---
